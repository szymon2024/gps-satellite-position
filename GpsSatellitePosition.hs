...
gpsSatellitePosition  t Ephemeris{..} =
  let
    ...
    xk = xk' * cos omegak - yk' * cos ik * sin omegak     -- EN: xk' transformation to ECEF
                                                          -- PL: transformacja xk' do ECEF
    yk = xk' * sin omegak + yk' * cos ik * cos omegak     -- EN: yk' transformation to ECEF
...
gpsTimeToWeekSow :: Integer -> Int -> Int -> Int -> Int -> Double -> (Integer, Double)
gpsTimeToWeekSow y mo d h m s =
    let day = fromGregorian y mo d
        ...
        gpsDayOfWeek = fromEnum (dayOfWeek day)
        wsec = fromIntegral (gpsDayOfWeek * 86400 + h * 3600 +  m * 60)
               + s
    in (week, wsec)
...
main = do
  let eph              = ephExample
      (gpsWeek,gpsSow) = gpsTimeToWeekSow 2024 03 07 21 59 30.0
      (xS,yS,zS)       = gpsSatellitePosition gpsSow eph
  if isEphemerisValid gpsWeek gpsSow eph then do
