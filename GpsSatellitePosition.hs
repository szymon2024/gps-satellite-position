...
-- | PL: Efemeryda GPS (podzbiór pól niezbędnych do wyznaczenia pozycji satelity)
...
fullWeekInS, halfWeekInS :: Double
fullWeekInS = 604800.0  -- EN: number of seconds of full week
                        -- PL: liczba sekund całego tygodnia
halfWeekInS = 302400.0  -- EN: number of seconds of half week
...
isEphemerisValid gpsWeek sow eph =
    let dt = wrapWeekCrossover $
               (sow + fromIntegral gpsWeek * fullWeekInS)
             - (toe eph + (week eph) * fullWeekInS)
...
main = do
  let Just gpsT  = mkGPSTime 2024 03 07 21 59 30.0
      ...
      (xS,yS,zS) = gpsSatellitePosition (realToFrac sow) eph
  if isEphemerisValid gpsWeek (realToFrac sow) eph then do
